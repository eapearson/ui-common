...
var app = angular.module('kbase-app',
...
window.configJSON = $.parseJSON($.ajax({url: "config.json",
    async: false,
    dataType: 'json'}).responseText);


// The current configuration key is stored on the "setup" property.
var currentConfig = window.configJSON[window.configJSON.setup];
...
function (Postal, Navbar, Logger, Notifications) {
    ...
    app.run(function ($rootScope, $state, $stateParams, $location) {
        ...
        window.USER_TOKEN = KBaseSessionSync.getAuthToken();
        window.USER_ID = KBaseSessionSync.getUsername();
        ...
        $rootScope.kb = window.kb;
        $rootScope.Object = Object;

        // This will be the first usage of Navbar, this being the app and all.
        Navbar.init({container: '#navbar'})
            .start()
            .then(function (nav) {
                nav.clear();
                nav.addDefaultMenu({
                ...
                $rootScope.$on('$locationChangeSuccess', function (e, next, current) {
                    ...
                    if (next !== current) {
                        Navbar.clear()
                            .addDefaultMenu();
                        // Gosh darnit, we need a widget for the window as well?
            ...
            .then (function () {
                // a cheap heartbeat for now... and just for the dashboard.
                var heartbeat = 0;
    ...
    angular.element(document).ready(function () {
        angular.bootstrap(document, ['kbase-app']);
    });
});
